...
-(bool) readTrackData:(ITrack*) track from:(NSMutableData*) buffer
{
	unsigned int len=0;
	Chunk chunk;
...
- (MidiFile*) load:(NSMutableData*) buffer//, unsigned int size) 
{
    BOOL ret=NO;
	int fmt;
...
- (MidiFile *)loadWithMidName:(NSString *)midName type:(NSString *)midType {
    NSMutableData *data = [NSMutableData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:midName ofType:midType]];
    if (data) {
...
- (MidiFile*)loadFromFileName:(NSString*)fileName fileType:(NSString *)fileType filePath:(NSString *)filePath
{
    ...
    NSString *path = [[documentDir stringByAppendingPathComponent:filePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@", fileName, fileType]];
...
- (void) writeMetaEvent:(NSMutableData*)buffer 
{
    ...
    int i;
    int trunk_start_pos=buffer.length;
    ...
    if (buffer.length >= 8) {
		unsigned int i;
		unsigned int size = buffer.length - trunk_start_pos - 8;
        char data[4];
		for (i = 0; i < 4; ++i) {
			data[3 - i] = size % 256;
			size /= 256;
